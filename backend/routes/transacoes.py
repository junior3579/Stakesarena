...
def solicitar_transacao():
    ...
    usuario = executar_query_fetchall(
        "SELECT nome, whatsapp, pontos FROM usuarios WHERE id = %s",
        (id_usuario,)
    )
    ...
    nome_usuario, whatsapp_usuario, pontos_usuario = usuario[0]
    
    # Verificar se tem saldo suficiente para saque
    if tipo == 'saque' and pontos_usuario < valor_int:
    ...
    mensagem += f"💰 *Valor:* {valor_int} pontos\n"
