...
def validar_pontos(pontos):
    try:
        ...
        if pontos_int < 0:
            return None, "O valor de pontos não pode ser negativo"
        if pontos_int % 2 != 0:
...
def listar_usuarios():
    usuarios = executar_query_fetchall("SELECT id, nome, pontos, whatsapp, posicao, pix_tipo, pix_chave, senha FROM usuarios ORDER BY posicao ASC")
    if not usuarios:
        return jsonify([])
...
def cadastrar_usuario():
    ...
    if not nome or not senha or pontos is None:
    ...
    sucesso = executar_query_commit(
        "INSERT INTO usuarios (id, nome, senha, pontos, whatsapp, posicao, pix_tipo, pix_chave) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
        (id_vago, nome, senha, pontos_validos, whatsapp, posicao_vaga, pix_tipo, pix_chave)
    )
...
def editar_usuario(id_usuario):
    ...
    sucesso = executar_query_commit(
        "UPDATE usuarios SET nome = %s, senha = %s, pontos = %s, whatsapp = %s, pix_tipo = %s, pix_chave = %s WHERE id = %s",
        (nome, senha, pontos_validos, whatsapp, pix_tipo, pix_chave, id_usuario)
    )
