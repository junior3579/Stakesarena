...
const UserDashboard = ({ user }) => {
  ...
  const [novaSala, setNovaSala] = useState({ nome_sala: '', valor_inicial: '', whatsapp: '', categoria_id: '' })
  ...
  const carregarDados = async () => {
    try {
      const [salasRes, categoriasRes, torneiosRes] = await Promise.all([
        fetch('/api/salas'),
  ...
  const criarSala = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setNovaSala({ nome_sala: '', valor_inicial: '', whatsapp: '', categoria_id: '' })
        carregarDados()
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="p-6 flex items-center space-x-3">
          ...
          <div><p className="text-sm text-gray-500">Saldo atual</p><p className="text-2xl font-bold">{saldoAtual} pontos</p></div>
        </Card>
        <OnlineUsers />
      </div>
            ...
            {salasFiltradas.map(sala => (
              <Card key={sala.id_sala} className="p-4 hover:shadow-md transition-shadow">
                <div className="flex justify-between items-start">
                  ...
                  <Badge className="bg-blue-600">{sala.valor_inicial} pts</Badge>
                </div>
                <div className="mt-4 flex items-center justify-between">
                  <span className="text-sm text-gray-600 flex items-center gap-1">
                    <Users className="h-3 w-3" /> {Object.keys(sala.jogadores).length}/2
                  </span>
                  <Button size="sm" onClick={() => entrarNaSala(sala.id_sala)} disabled={Object.keys(sala.jogadores).length >= 2}>
                    Entrar ({sala.valor_inicial / 2} pts)
                  </Button>
                </div>
              </Card>
            ))}
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    ...
                    {t.valor_inscricao > 0 && <Badge className="bg-blue-600">Inscrição: {t.valor_inscricao} pts</Badge>}
                    {t.premio > 0 && <Badge className="bg-green-600">Prêmio: {t.premio} pts</Badge>}
                  </div>
                  ...
                  <Button 
                    ...
                    onClick={() => {
                      fetch(`/api/torneios/${t.id}/inscrever`, {
                        ...
                        else { toast.success(data.message); carregarDados() }
                      })
                    }}
                    disabled={t.participantes.some(p => p.id === user.id)}
                  >
                    {t.participantes.some(p => p.id === user.id) ? 'Você já está inscrito' : `Inscrever-se (${t.valor_inscricao} pts)`}
                  </Button>
                )}
          ...
          <Card className="p-4">
            ...
            <form onSubmit={criarSala} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input value={novaSala.whatsapp} onChange={e => setNovaSala({...novaSala, whatsapp: e.target.value})} placeholder="Ex: 11999999999" />
              </div>
